    ...
    def show(String id) {
        ...
        try {
            ...
            if (record) {
                ...
                // adds some links to certain fields, etc
                compareRecord = postProcessingService.augmentRecord(compareRecord)

                String plutofURL = ""
                if (record.raw.occurrence?.occurrenceID && record.processed.attribution?.dataProviderUid && record.processed.attribution?.dataProviderUid == grailsApplication.config?.dataProvider.plutof) {
                    String bor = borPlutofMap.get(record.raw.occurrence?.basisOfRecord)
                    if (bor) {
                        plutofURL = "https://plutof.ut.ee/#/${bor}/view/${record.raw.occurrence.occurrenceID}"
                    }
                }
