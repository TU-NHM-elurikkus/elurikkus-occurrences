...
$(document).ready(function() {
        ...
        if(text) {
            $.each(text, function(j, el) {
                ...
                if(sensitiveDatasets[code]) {
                    var linked =
                    '<a href="' + sensitiveDatasets[code] + '" title="' + list
    ...
    $('.dataQualityHelpLinkZZZ').click(function(e) {
        ...
        if(dataQualityItem) {
            content =
            '<div>' +
                '<b>Name: ' + dataQualityItem.name + '</b>' +
            '</div>' +
            '<div>' +
                dataQualityItem.description +
            '</div>' +
            '<div>' +
                '<a href="https://github.com/AtlasOfLivingAustralia/ala-dataquality/wiki/' + dataQualityItem.name + '" target="wiki" title="More details on the wiki page">' +
                    'Wiki page' +
                '</a>' +
            '</div>';
        }
...
function refreshUserAnnotations() {
    $.get(OCC_REC.contextPath + '/assertions/' + OCC_REC.recordUuid, function(data) {
        ...
        for(i = 0; i < data.assertionQueries.length; i++) {
            $clone = $('#userAnnotationTemplate').clone();
        ...
        for(i = 0; i < data.userAssertions.length; i++) {
        ...
        for(i = 0; i < sortedVerifiedAssertion.length; i++) {
        ...
        for(i = 0; i < disableDelete.length; i++) {
...
function updateDeleteEvents(enableDelete, disableDelete) {
    ...
    for(i = 0; i < enableDelete.length; i++) {
